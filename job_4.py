...
spark = SparkSession\
    .builder\
    .appName("job_4")\
    .config("spark.cassandra.connection.host","10.140.10.2") \
...
try:
    print("\n\tLendo Parquets...")
    inicio = time.time()
    df_autuacao = Parquet('gs://parquets/autuacao').ler_parquet(spark)
    df_beneficiada = Parquet('gs://parquets/beneficiada').ler_parquet(spark)
    df_pib = Parquet('gs://parquets/pib').ler_parquet(spark)
    df_arrecadacao = Parquet('gs://parquets/arrecadacao').ler_parquet(spark)
    df_distribuicao = Parquet('gs://parquets/distribuicao').ler_parquet(spark)
    df_barragens = Parquet('gs://parquets/barragens').ler_parquet(spark)
    df_municipio = Parquet('gs://parquets/municipio').ler_parquet(spark)   

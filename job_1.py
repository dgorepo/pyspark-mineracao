...
try:
    print("\n\tLendo CSVs...")
    inicio = time.time()
    df_arrecadacao = CSV(";", "ISO-8859-1", "gs://datasets/arrecadacao.csv", schema_arrecadacao).ler_csv(spark)
    df_autuacao = CSV(";", "ISO-8859-1", "gs://datasets/autuacao.csv", schema_autuacao).ler_csv(spark)
    df_barragens = CSV(";", "ISO-8859-1", "gs://datasets/barragens.csv", schema_barragens).ler_csv(spark)
    df_beneficiada = CSV(",", "ISO-8859-1", "gs://datasets/beneficiada.csv", schema_beneficiada).ler_csv(spark)
    df_distribuicao = CSV(";", "ISO-8859-1", "gs://datasets/distribuicao.csv", schema_distribuicao).ler_csv(spark)
    df_municipio = CSV(";", "ISO-8859-1", "gs://datasets/municipio.csv", schema_municipio).ler_csv(spark)
    df_pib = CSV(",", "ISO-8859-1", "gs://datasets/pib.csv", schema_pib).ler_csv(spark)
